...
import PetCargoForm from './PetCargoForm'; // <-- Hier importiert!
            ...
            {passengerDetails.map((p: any, idx: number) => (
              <div key={p.id} className="p-4 bg-slate-50 rounded-2xl border space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input type="text" required placeholder="Vorname" value={p.firstName} onChange={(e) => updatePassenger(p.id, 'firstName', e.target.value)} className="border p-2.5 rounded-xl text-xs font-bold" />
                  <input type="text" required placeholder="Nachname" value={p.lastName} onChange={(e) => updatePassenger(p.id, 'lastName', e.target.value)} className="border p-2.5 rounded-xl text-xs font-bold" />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <input type="date" required value={p.birthDate} onChange={(e) => updatePassenger(p.id, 'birthDate', e.target.value)} className="border p-2.5 rounded-xl text-xs font-bold cursor-pointer" />
                  <input type="text" required placeholder="Nationalität" value={p.nationality} onChange={(e) => updatePassenger(p.id, 'nationality', e.target.value)} className="border p-2.5 rounded-xl text-xs font-bold" />
                </div>
                <input type="text" required placeholder="Ausweis- / Reisepassnummer" value={p.passport} onChange={(e) => updatePassenger(p.id, 'passport', e.target.value)} className="w-full border p-2.5 rounded-xl text-xs font-mono font-bold" />
              </div>
            ))}
          </div>
      ...
      {subStage === 'step6_extras' && (
        <div className="bg-white rounded-3xl p-6 border shadow-xl space-y-4 max-w-md mx-auto">
          ...
          <div className="space-y-2.5">
            <button type="button" onClick={() => setSelectedCabin(!selectedCabin)} className={`w-full p-3.5 rounded-xl border-2 flex justify-between items-center font-bold text-xs transition-all ${selectedCabin ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 border-slate-200'}`}>
              <span>🛌 Private Schlafkabine</span><span className="font-mono text-xs font-black">+{selectedOffer?.cabinPrice || 60} €</span>
            </button>
            <button type="button" onClick={() => setSelectedMeal(!selectedMeal)} className={`w-full p-3.5 rounded-xl border-2 flex justify-between items-center font-bold text-xs transition-all ${selectedMeal ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 border-slate-200'}`}>
            ...
            {/* Bindet das Haustierausweis-Zollformular live ein */}
